...
NAMESPACE_BEGIN(filesystem)
	...
	public:
		...
		path(path&& path) noexcept : m_type(path.m_type), m_path(std::move(path.m_path)),
		...
		void set(const std::wstring& wstring, path_type type = native_path) {
			std::string string;
			if (!wstring.empty()) {
				int size = WideCharToMultiByte(CP_UTF8, 0, &wstring[0], static_cast<int>(wstring.size()),
				                               nullptr, 0, nullptr, nullptr);
				string.resize(size, 0);
				WideCharToMultiByte(CP_UTF8, 0, &wstring[0], static_cast<int>(wstring.size()),
