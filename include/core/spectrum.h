...
M_NAMESPACE_BEGIN
	...
	public:
		...
		template <typename, int, DeviceType>
		friend class TRGBSpectrum;
...
#else
			if constexpr (Device == DeviceType::CPU) {
				assert(false && "Error: Can not construct Spectrum on CPU by device function");
			} else {
		...
		M_HOST_DEVICE explicit TRGBSpectrum(Scalar* input_data, DeviceType input_device) {
#ifndef __CUDA_ARCH__
			if constexpr (Device == DeviceType::CPU) {
			...
			if constexpr (Device == DeviceType::CPU) {
				...
				if (input_device == DeviceType::CPU) {
					assert(false && "Error: Can not construct Spectrum on GPU by device function through a CPU m_data");
				} else {
					for (int i = 0; i < Dimension; i++) {
		...
		M_HOST_DEVICE explicit TRGBSpectrum(TRGBSpectrum<Scalar, OtherDimension, Device> other_spectrum,
		                                    Scalar padding = 1) {
#ifndef __CUDA_ARCH__
			...
			if constexpr (Device == DeviceType::CPU) {
				...
				check_cuda_error(cudaMemcpyAsync(m_data, other_spectrum.m_data, CopyDimension * sizeof(Scalar),
				                                 cudaMemcpyDeviceToDevice), "Memory copy failed");
				check_cuda_error(
					cudaMemset(m_data + CopyDimension, padding, (Dimension - CopyDimension) * sizeof(Scalar)),
					"Memory set failed");
			}
#else
			if constexpr (Device == DeviceType::CPU) {
...
#else
			if constexpr (Device == DeviceType::CPU) {
				assert(false && "Error: Cannot construct Spectrum on CPU in device function");
			} else {
				for (auto it = values.begin(); it != values.end() && i < Dimension; ++it, ++i) {
					m_data[i] = *it;
				}
		...
		M_HOST_DEVICE TRGBSpectrum<Scalar, Dimension - 1, Device> divide_by_weight() {
