...
M_NAMESPACE_BEGIN
template <typename Scalar_> class TFrame {
public:
    typedef Scalar_ Scalar;
    typedef TArray<Scalar, 3, ArrayType::Vector> VectorType;
    typedef TArray<Scalar, 2, ArrayType::Vector> VectorType2;
    ...
    explicit TFrame() : n(NormalType(static_cast<Scalar>(0), static_cast<Scalar>(0), static_cast<Scalar>(1))) {
        bool valid = true;
    ...
    TFrame(const VectorType &s, const VectorType &t, const NormalType &n) : s(s), t(t), n(n) {
        bool valid = true;
        this->n.self_norm(valid);
        this->s.self_norm(valid);
        this->t.self_norm(valid);
    ...
    explicit TFrame(const NormalType &n) : n(n) {
        bool valid = true;
        this->n.self_norm(valid);
    ...
    // ============================== Function ==============================
    void coordinate_system(const NormalType &n, VectorType &b, VectorType &c, bool &valid) {
    ...
    [[nodiscard]] VectorType to_local(const VectorType &v) const { return { v.dot(s), v.dot(t), v.dot(n) }; }

    // Convert from local coordinates to world coordinates
    [[nodiscard]] VectorType to_world(const VectorType &v) const {
    ...
    static std::pair<Scalar, Scalar> sincos_phi(const VectorType &v) {
        ...
        result =
            abs(sin_theta_2) <= Scalar(4) * M_EPSILON ? VectorType2({ 1.f, 0.f }) : result.clamp(Scalar(-1), Scalar(1));
    ...
    // Inequality test
    bool operator!=(const TFrame &frame) const { return !operator==(frame); }
