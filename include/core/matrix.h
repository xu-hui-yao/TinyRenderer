	...
	public:
		...
		M_HOST_DEVICE explicit TMatrix(Scalar value = 0) {
			...
			if constexpr (Device == DeviceType::CPU) {
				...
				check_cuda_error(
					cudaMemcpyAsync(m_data, temp, Rows * Cols * sizeof(Scalar), cudaMemcpyHostToDevice),
					"Memory copy failed");
				delete[] temp;
			}
#else
		...
		M_HOST_DEVICE explicit TMatrix(Scalar* input_m_data, DeviceType input_device) {
#ifndef __CUDA_ARCH__
			if constexpr (Device == DeviceType::CPU) {
				if (input_device == DeviceType::CPU) {
					...
					check_cuda_error(
						cudaMemcpyAsync(m_data, input_m_data, Rows * Cols * sizeof(Scalar), cudaMemcpyDeviceToHost),
						"Memory copy failed");
				}
			} else {
				if (input_device == DeviceType::CPU) {
					check_cuda_error(
						cudaMemcpyAsync(m_data, input_m_data, Rows * Cols * sizeof(Scalar), cudaMemcpyHostToDevice),
						"Memory copy failed");
				} else {
					check_cuda_error(
						cudaMemcpyAsync(m_data, input_m_data, Rows * Cols * sizeof(Scalar), cudaMemcpyDeviceToDevice),
						"Memory copy failed");
				}
			}
#else
			if constexpr (Device == DeviceType::CPU) {
				...
				if (input_device == DeviceType::CPU) {
					printf("Error: Can not construct Matrix on GPU by device function through a CPU m_data");
				} else {
					for (int i = 0; i < Rows * Cols; i++) {
		...
		M_HOST_DEVICE TMatrix(const TMatrix& other) {
			...
			if constexpr (Device == DeviceType::CPU) {
				...
				assert(false && "Error: Can not construct Matrix on GPU by host function");
			}
#else
		...
		M_HOST_DEVICE TMatrix& operator=(const TMatrix& other) {
			...
			if constexpr (Device == DeviceType::CPU) {
				...
				check_cuda_error(
					cudaMemcpyAsync(m_data, other.m_data, Rows * Cols * sizeof(Scalar), cudaMemcpyDeviceToDevice),
		...
		M_HOST_DEVICE TMatrix<Scalar, ViewRow, ViewCol, Device> view(
		...
		M_HOST_DEVICE TMatrix<Scalar, Rows, OtherCols, Device> operator*(
			const TMatrix<Scalar, Cols, OtherCols, Device>& other) const {
		...
		template <int OtherCols>
		M_HOST_DEVICE TMatrix<Scalar, Rows, OtherCols, Device>& operator*=(
			const TMatrix<Scalar, Cols, OtherCols, Device>& other) {
			*this = *this * other;
			return *this;
		}
		...
		template <ArrayType OtherArrayType>
		M_HOST_DEVICE TArray<Scalar, Rows, OtherArrayType, Device>& operator*=(
			const TArray<Scalar, Cols, OtherArrayType, Device>& vector) {
			*this = *this * vector;
			return *this;
		}
		...
		// Comparator
		M_HOST_DEVICE TMatrix<int, Rows, Cols, Device> operator<(const TMatrix& other) const {
		...
		[[nodiscard]] std::string to_string() const {
			...
			if constexpr (Device == DeviceType::GPU) {
				check_cuda_error(cudaMemcpyAsync(temp, m_data, Rows * Cols * sizeof(Scalar), cudaMemcpyDeviceToHost));
