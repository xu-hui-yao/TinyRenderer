...
M_NAMESPACE_BEGIN
	...
	public:
		...
		M_HOST_DEVICE explicit TMicrofacetDistribution(const TMicrofacetDistribution<Scalar, Device>& other) {
			m_alpha = other.m_alpha;
			configure();
		}

		M_HOST_DEVICE explicit TMicrofacetDistribution(Scalar alpha) : m_alpha(alpha) {
			configure();
		}
		...
		M_HOST_DEVICE std::pair<Normal, Scalar> sample(const Vector& wi, const Point2& sample, bool& valid) const {
		...
		M_HOST_DEVICE Scalar g(const Vector& wi, const Vector& wo, const Normal& m) const {
		...
		M_HOST_DEVICE void scale_alpha(Scalar value) {
			m_alpha *= value;
		}

	private:
		M_HOST_DEVICE void configure() {
			m_alpha = M_MAX(m_alpha, Scalar(1e-4));
		}

		// \brief Visible normal sampling code for the alpha=1 case
		M_HOST_DEVICE Vector2 sample_visible_11(Scalar cos_theta_i, const Point2& sample) const {
