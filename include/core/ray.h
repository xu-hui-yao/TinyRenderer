...
M_NAMESPACE_BEGIN
	...
	public:
		...
		M_HOST_DEVICE TRay(const Point& origin, const Vector& direction, bool& valid)
			: m_origin(origin), m_min_t(M_EPSILON), m_max_t(M_MAX_FLOAT) {
			this->m_direction = direction.norm(valid);
			update(valid);
		}

		M_HOST_DEVICE TRay(const Point& origin, const Vector& direction, Scalar min_t, Scalar max_t, bool& valid)
		...
		M_HOST_DEVICE Point o() const {
			return m_origin;
		}

		M_HOST_DEVICE Point& o() {
			return m_origin;
		}

		M_HOST_DEVICE Vector d() const {
			return m_direction;
		}

		M_HOST_DEVICE Vector& d() {
			return m_direction;
		}

		M_HOST_DEVICE Vector d_reciprocal() const {
			return m_direction_reciprocal;
		}

		M_HOST_DEVICE Vector& d_reciprocal() {
			return m_direction_reciprocal;
		}

		M_HOST_DEVICE Scalar min_t() const {
			return m_min_t;
		}

		M_HOST_DEVICE Scalar& min_t() {
			return m_min_t;
		}

		M_HOST_DEVICE Scalar max_t() const {
			return m_max_t;
		}

		M_HOST_DEVICE Scalar& max_t() {
			return m_max_t;
		}
