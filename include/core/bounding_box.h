...
M_NAMESPACE_BEGIN
	...
	public:
		...
		M_HOST_DEVICE explicit TBoundingBox(const Point& p) : m_min_p(p), m_max_p(p) {}

		M_HOST_DEVICE TBoundingBox(const Point& min, const Point& max) : m_min_p(min), m_max_p(max) {}

		// ============================== Getter and setter ==============================

		M_HOST_DEVICE Point& get_min() {
			return m_min_p;
		}

		M_HOST_DEVICE Point get_min() const {
			return m_min_p;
		}

		M_HOST_DEVICE Point& get_max() {
			return m_max_p;
		}

		M_HOST_DEVICE Point get_max() const {
			return m_max_p;
		}
		...
		M_HOST_DEVICE Vector get_extents() const {
		...
		M_HOST_DEVICE static TBoundingBox merge(const TBoundingBox& bbox1, const TBoundingBox& bbox2) {
			return TBoundingBox(
				bbox1.m_min_p.wise_min(bbox2.m_min_p),
	...
	M_HOST_DEVICE [[nodiscard]] bool ray_intersect(const Ray3f& ray) const {
		...
		M_HOST_DEVICE bool ray_intersect(const Ray3f& ray, Scalar& near_t, Scalar& far_t) const {
			near_t = -M_MAX_FLOAT;
			far_t = M_MAX_FLOAT;
