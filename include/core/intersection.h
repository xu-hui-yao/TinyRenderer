 ...
 */
template <typename Scalar_, DeviceType Device_> class TIntersection {
public:
    ...
    M_HOST_DEVICE
    TIntersection(const PointType &_p, const Scalar &_t, const NormalType &_n)
        : p(_p), t(_t), n(_n) {}

    M_HOST_DEVICE TIntersection() : p(PointType(0)), t(M_MAX_FLOAT), n(1) {}

    M_HOST_DEVICE TIntersection &
    operator=(const TSurfaceIntersection<Scalar, Device> &other) {
        p = other.p;
        t = other.t;
        n = other.n;
        return *this;
...
public:
    ...
    M_HOST_DEVICE explicit TSurfaceIntersection(
        const TPositionSample<Scalar, Device> &ps)
        : Base(ps.p, 0, ps.n), uv(ps.uv), shading_frame(FrameType(ps.n)),
          wi(VectorType(0)), dp_du(VectorType(0)), dp_dv(VectorType(0)) {}
