 ...
 */
template <typename Scalar_, DeviceType Device_> class TPositionSample {
public:
    ...
    bool delta = false; // Set if the sample was drawn from a degenerate (Dirac
                        // delta) distribution

    M_HOST_DEVICE TPositionSample() = default;

    M_HOST_DEVICE explicit TPositionSample(
        const TDirectionSample<Scalar, Device> other) {
        p     = other.p;
        n     = other.n;
        uv    = other.uv;
        t     = other.t;
    ...
    M_HOST_DEVICE explicit TPositionSample(
        const TSurfaceIntersection<Scalar, Device> &si)
        : p(si.p), n(si.n), uv(si.uv), t(si.t), pdf(0), delta(false) {}

    M_HOST_DEVICE explicit TPositionSample(const PointType &_p,
                                           const NormalType &_n,
                                           const PointType2 &_uv, Scalar _t,
                                           Scalar _pdf, bool _delta)
...
public:
     ...
     *     Reference position
     */
    M_HOST_DEVICE explicit TDirectionSample(
        const TSurfaceIntersection<Scalar, Device> &si,
        const TIntersection<Scalar, Device> &ref)
        : Base(si) {
    ...
    M_HOST_DEVICE explicit TDirectionSample(
        const Point3f &p, const Normal3f &n, const Point2f &uv, const Scalar &t,
        const Scalar &pdf, const bool &delta, const Vector3f &d,
        const Scalar &dist, const std::shared_ptr<Emitter> &emitter)
...
template <typename Scalar_, DeviceType Device_> class TBSDFSample {
public:
    static constexpr DeviceType Device = Device_;
    ...
    M_HOST_DEVICE explicit TBSDFSample(const Vector3f &wo)
        : wo(wo), pdf(0.f), eta(1.f) {}
};
