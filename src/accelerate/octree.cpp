...
M_NAMESPACE_BEGIN
	...
	public:
		...
		[[nodiscard]] bool ray_test(const Ray3f& ray) const override {
			return ray_test_node(ray, root);
		}

		[[nodiscard]] std::string to_string() const override {
			return "Octree Accelerate\n" + node_to_string(root, 0);
		}
	...
	private:
		...
		std::shared_ptr<OctreeNode> build_tree(const BoundingBox3f& bbox,
		                                       const std::vector<std::pair<std::shared_ptr<Mesh>, uint32_t>>&
		                                       primitive_list, int depth) {
