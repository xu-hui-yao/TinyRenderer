...
std::shared_ptr<Object> load_from_xml(const std::string &filename) {
    ...
    if (!result) {
        throw std::runtime_error("Error while parsing " + filename + ": " + result.description() + " (at " +
    ...
    auto check_attributes = [&](const pugi::xml_node &node, std::set<std::string> attrs) {
        for (auto attr : node.attributes()) {
            ...
            if (it == attrs.end()) {
                throw std::runtime_error("Error while parsing " + filename + ": unexpected attribute " + attr.name() +
                                         " in " + node.name() + " at " + offset(node.offset_debug()));
            }
            attrs.erase(it);
        }
        if (!attrs.empty()) {
            throw std::runtime_error("Error while parsing " + filename + ": missing attribute " + *attrs.begin() +
                                     " in " + node.name() + " at " + offset(node.offset_debug()));
        }
    };
    ...
    std::function<std::shared_ptr<Object>(pugi::xml_node &, PropertyList &, int)> parse_tag =
        ...
        if (node.type() != pugi::node_element) {
            throw std::runtime_error("Error while parsing " + filename + ": unexpected content at " +
        ...
        if (it == tags.end()) {
            throw std::runtime_error("Error while parsing " + filename + ": unexpected tag " + node.name() + " at " +
                                     offset(node.offset_debug()));
        }
        int tag = it->second;
        ...
        bool current_is_transform_op =
            tag == ETranslate || tag == ERotate || tag == EScale || tag == ELookAt || tag == EMatrix;
        ...
        if (has_parent && !parent_is_object && !(parent_is_transform && current_is_transform_op)) {
            throw std::runtime_error("Error while parsing " + filename + ": node " + node.name() +
        ...
        try {
            if (current_is_object) {
                ...
                if (result_->get_class_type() != tag) {
                    throw std::runtime_error("Unexpectedly constructed an object "
                                             "of type <" +
                                             Object::class_type_name(result_->get_class_type()) + "> (expected type <" +
                                             Object::class_type_name(static_cast<Object::EClassType>(tag)) +
                                             ">): " + result_->to_string());
                }
                ...
                switch (tag) {
                    ...
                    default:
                        throw std::runtime_error("Unhandled element " + std::string(node.name()));
                };
            }
        } catch (const std::exception &e) {
            throw std::runtime_error("Error while parsing " + filename + ": " + e.what() + " (at " +
                                     offset(node.offset_debug()) + ")");
        }
