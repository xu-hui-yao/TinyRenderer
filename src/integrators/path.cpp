...
M_NAMESPACE_BEGIN
	...
	public:
		...
		[[nodiscard]] Color3f li(const std::shared_ptr<Scene>& scene, std::shared_ptr<Sampler> sampler,
			...
			for (uint32_t i = 0; i < m_max_depth && valid; i++) {
				...
				bool active_next = depth + 1 < m_max_depth && its.is_valid();
				...
				auto bsdf_val = bsdf->eval(its, wo, active_em);
				auto bsdf_pdf = bsdf->pdf(its, wo, active_em);
