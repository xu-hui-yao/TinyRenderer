...
M_NAMESPACE_BEGIN
	...
	public:
		...
		void add_child(const std::shared_ptr<Object>& child) override {
			...
			default:
				throw std::runtime_error(
					"BumpMap::add_child(<" + class_type_name(child->get_class_type()) + ">) is not supported!");
			}
		}
		...
		[[nodiscard]] std::string to_string() const override {
			return "BumpMap[\n"
				"  nested_texture = " + indent(m_nested_texture ? m_nested_texture->to_string() : "null") + "\n"
				"  nested_bsdf = " + indent(m_nested_bsdf ? m_nested_bsdf->to_string() : "null") + "\n"
