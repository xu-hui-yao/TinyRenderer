...
M_NAMESPACE_BEGIN
	...
	public:
		...
		[[nodiscard]] std::pair<BSDFSample3f, Color3f> sample(const SurfaceIntersection3f& si,
			...
			// Trick by Walter et al.: slightly scale the roughness values to reduce importance sampling weights. Not needed for the Heitz and D'Eon sampling technique.
		...
		[[nodiscard]] float pdf(const SurfaceIntersection3f& si, const Vector3f& wo, bool active) const override {
			...
			active &= si.wi.dot(m) * Frame3f::cos_theta(si.wi, active) > 0.f &&
				wo.dot(m) * Frame3f::cos_theta(wo, active) > 0.f;
