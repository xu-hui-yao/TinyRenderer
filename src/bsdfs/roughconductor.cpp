...
public:
    explicit RoughConductor(const PropertyList &properties)
        : m_eta(nullptr), m_k(nullptr), m_alpha(nullptr), m_specular_reflectance(nullptr) {
    ...
    [[nodiscard]] std::pair<BSDFSample3f, Color3f> sample(const SurfaceIntersection3f &si, float /* sample1 */,
        ...
        Color3f f({ fresnel_conductor(si.wi.dot(m), m_eta->eval(si, active)(0), m_k->eval(si, active)(0)),
