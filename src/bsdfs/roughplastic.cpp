...
M_NAMESPACE_BEGIN
	...
	public:
		explicit RoughPlastic(const PropertyList& properties) : m_diffuse_reflectance(nullptr),
    ...
    std::cout << "Construct " << class_type_name(get_class_type()) << std::endl;
		...
		[[nodiscard]] Color3f eval(const SurfaceIntersection3f& si,
			...
			diffuse /= Color3f(1.0f) - (
				m_nonlinear ? diffuse * m_internal_reflectance : Color3f(m_internal_reflectance));
		...
		[[nodiscard]] std::string to_string() const override {
			...
			oss << "RoughPlastic[" << std::endl
				<< "  diffuse_reflectance = " << indent(m_diffuse_reflectance
					                                        ? m_diffuse_reflectance->to_string()
					                                        : "null")
				<< "," << std::endl
				<< "  specular_reflectance = " << indent(m_specular_reflectance
					                                         ? m_specular_reflectance->to_string()
					                                         : "null") << "," << std::endl
