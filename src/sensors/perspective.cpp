...
public:
    explicit PerspectiveCamera(const PropertyList &property_list) {
        ...
        inverse_output_size =
            Vector2f(1.0f / static_cast<float>(output_size.x()), 1.0f / static_cast<float>(output_size.y()));
    ...
    void construct() override {
         ...
         */
        float recip = 1.0f / (far_clip - near_clip), cot = 1.0f / tan(deg_to_rad(fov / 2.0f));
        ...
        sample_to_camera = Transform4f(Matrix4f::scale(Vector4f(-0.5f, -0.5f * aspect, 1.0f, 1.0f)) *
                                       Matrix4f::translate(Vector4f(-1.0f, -1.0f / aspect, 0.0f, 0.0f)) * perspective)
                               .inverse();
    ...
    void add_child(const std::shared_ptr<Object> &child) override {
        switch (child->get_class_type()) {
            ...
            default:
                throw std::runtime_error("Camera::add_child(<" + class_type_name(child->get_class_type()) +
                                         ">) is not supported!");
        }
    }
    ...
    [[nodiscard]] std::string to_string() const override {
        return std::string("PerspectiveCamera[\n") + std::string("  c2w = ") + indent(camera_to_world.to_string()) +
               std::string(",\n") + std::string("  output size = ") + indent(output_size.to_string()) +
               std::string(",\n") + std::string("  fov = ") + std::to_string(fov) + std::string(",\n") +
               std::string("  clip = [near = ") + std::to_string(near_clip) + std::string(", far = ") +
               std::to_string(far_clip) + std::string("],\n") + std::string("  reconstruction filter = ") +
               (m_reconstruction_filter ? indent(m_reconstruction_filter->to_string()) : std::string("None")) +
               std::string("\n]");
    }
