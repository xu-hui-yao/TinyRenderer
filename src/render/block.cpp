...
M_NAMESPACE_BEGIN
	...
	void ImageBlock::put(const Point2f& _pos, const Color3f& value) {
		...
		Point2f pos(
			_pos.x() - 0.5f - static_cast<float>(m_offset.x() - m_border_size),
			_pos.y() - 0.5f - static_cast<float>(m_offset.y() - m_border_size)
		);

		BoundingBox2i bbox(
			Point2i(static_cast<int>(std::ceil(pos.x() - m_filter_radius)),
	...
	BlockGenerator::BlockGenerator(const Vector2i& size, int block_size)
		: m_size(size), m_block_size(block_size) {
		m_num_blocks = Vector2i(
			static_cast<int>(std::ceil(static_cast<float>(size.x()) / static_cast<float>(block_size))),
	...
	bool BlockGenerator::has_next() const {
	...
	bool BlockGenerator::next(ImageBlock& block) {
		...
		do {
			switch (m_direction) {
			case ERight: ++m_block.x();
				break;
			case EDown: ++m_block.y();
				break;
			case ELeft: --m_block.x();
				break;
			case EUp: --m_block.y();
				break;
			default: break;
