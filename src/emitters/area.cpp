...
M_NAMESPACE_BEGIN
	...
	public:
		...
		void add_child(const std::shared_ptr<Object>& child) override {
			...
			default:
				throw std::runtime_error(
					"Area::add_child(<" + class_type_name(child->get_class_type()) + ">) is not supported!");
			}
		}
		...
		[[nodiscard]] std::pair<DirectionSample3f, Color3f> sample_direction(
			const Intersection3f& it, const Point2f& sample, bool& active) const override {
		...
		[[nodiscard]] std::pair<PositionSample3f, float> sample_position(
			const Point2f& sample, bool& active) const override {
