...
public:
        ...
        switch (child->get_class_type()) {
            ...
            default:
                throw std::runtime_error(
                    "Area::add_child(<" +
                    class_type_name(child->get_class_type()) +
                    ">) is not supported!");
        }
    }
    ...
    sample_direction(const Intersection3f &it, const Point2f &sample,
                     bool &active) const override {
        DirectionSample3f ds =
            std::dynamic_pointer_cast<Mesh>(parent)->sample_direction(
                it, sample, active);
        ...
        ds.emitter =
            std::dynamic_pointer_cast<Emitter>(std::make_shared<Area>(*this));

        return { ds, active ? spec : Color3f(0.0f) };
    }

    [[nodiscard]] float pdf_direction(const Intersection3f &it,
                                      const DirectionSample3f &ds,
                                      bool &active) const override {
        float dp = ds.d.dot(ds.n);
        active &= dp < 0.0f;
        float value = std::dynamic_pointer_cast<Mesh>(parent)->pdf_direction(
            it, ds, active);
        return active ? value : 0.0f;
    }

    [[nodiscard]] std::pair<PositionSample3f, float>
    sample_position(const Point2f &sample, bool &active) const override {
        PositionSample3f ps =
            std::dynamic_pointer_cast<Mesh>(parent)->sample_position(sample,
                                                                     active);
        float weight = active && ps.pdf > 0.f ? 1.0f / ps.pdf : 0.0f;
        return { ps, weight };
    }

    [[nodiscard]] float pdf_position(const PositionSample3f &ps,
                                     bool &active) const override {
        float pdf =
            std::dynamic_pointer_cast<Mesh>(parent)->pdf_position(ps, active);
        return active ? pdf : 0.0f;
    }

    [[nodiscard]] Color3f eval(const SurfaceIntersection3f &si,
                               bool &active) const override {
        return Frame3f::cos_theta(si.wi, active) > 0.0f
    ...
    [[nodiscard]] std::string to_string() const override {
        return std::string("Area[\n  radiance = ") +
               indent(m_radiance->to_string()) + "\n]";
    }

    [[nodiscard]] bool is_spatial_varying() const override {
        return m_radiance->is_spatial_varying();
    }
